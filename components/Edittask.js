...
class Edittask extends React.Component {
    ...
    state = {
        dealName:this.props.note.titleNote, 
        dealDescr:this.props.note.descriptNote,
        dealData:this.props.note.dataendNote,   
        dealPrior:this.props.note.priorNote,
        errorTitle:'',
        errorData:''  
    }
    ...
    trySaveEdit = () => {
        console.log("запустили сохранение");
        if (!this.state.dealName){
            ...
            errorTitle:'',
            errorData:''  })
            this.props.cbEditNote(note)
        }

    }

    render() {
        console.log('Запустили модуль Edittask')
    ...
    return (
        <div className='addtask'> 
            <h2>{this.props.titles.titleName18}</h2>
            <form>
            <div className='addRow'>
                <span className="addTitle">{this.props.titles.titleName6}</span>
                <input type="text" className='field' placeholder={this.props.titles.titleName6} onChange={this.setTitle} value={this.state.dealName}></input>
                <span className='errorRow'>{this.state.errorTitle}</span>
            </div>
            
            <div className='addRow'>
                <span className="addTitle">{this.props.titles.titleName7}</span>
                <textarea  className='field' placeholder={this.props.titles.titleName7} rows='10' onChange={this.setDescript} defaultValue={this.state.dealDescr}></textarea>
            </div>
            <div className='addRow'>
                <span className="addTitle">{this.props.titles.titleName8}</span>
                <input type="date" onChange={this.setDate} value={this.state.dealData}></input> <span className='errorRow'>{this.state.errorData}</span>
            </div>
            ...
            <div className='addButton'>
                <button type='button' value='save' onClick={this.trySaveEdit}>{this.props.titles.titleName10}</button> 
                <button type='reset' value='reset' onClick={this.resetNote}>{this.props.titles.titleName11}</button>
                <button type='button' value="cancel" onClick={this.cancelEdit}>{this.props.titles.titleName19}</button>
            </div></form>
        </div>        
      )  
    }
